...
export class AgentFactory {
    ...
    createAgent(): ReactAgentEngine {
        ...
        const llmAdapter: ILlmProvider = {
            chat: async (history: AgentMessage[], model?: string, tools?: any[]) => {
                console.log('[AgentFactory.LLMAdapter] chat called with', history.length, 'messages');
                const lastMessage = history[history.length - 1];
                const historyWithoutLast = history.slice(0, -1);
                console.log('[AgentFactory.LLMAdapter] Tools count:', tools?.length || 0);
                ...
                return this.llmService.chat(llmMessages, lastMessage.content, model, tools);
            },
            streamChat: async function* (history: AgentMessage[], model?: string, tools?: any[]) {
                console.log('[AgentFactory.LLMAdapter] streamChat called with', history.length, 'messages');
                const lastMessage = history[history.length - 1];
                console.log('[AgentFactory.LLMAdapter] Last message:', { role: lastMessage.role, content: lastMessage.content.substring(0, 100) });
                const historyWithoutLast = history.slice(0, -1);
                console.log('[AgentFactory.LLMAdapter] History length after pop:', historyWithoutLast.length);
                console.log('[AgentFactory.LLMAdapter] Tools count:', tools?.length || 0);
                console.log('[AgentFactory.LLMAdapter] Content to send:', lastMessage.content.substring(0, 100));
                ...
                yield* this.llmService.streamChat(llmMessages, lastMessage.content, model, tools);
            }.bind(this)
        };
    ...
    getTools(scope: string = 'global'): IToolProvider[] {
        ...
        const toolDefs = this.toolRegistry.getToolsDefinitions([scope]);
