...
import { DistillationPayload, DistillationResult } from '../worker/worker.interfaces';
 ...
 */
export async function processDistillation(task: DistillationPayload): Promise<DistillationResult> {
            ...
            if (config.DB_HOST) {
                try {
                    ...
                    for (const m of memories) {
                        if (m.entities && Array.isArray(m.entities) && m.relations && Array.isArray(m.relations)) {
