...
import { DistillationConfig, DistillationResult, DistillationPayload } from '../worker/worker.interfaces';
...
export class MemoryService {
    ...
    async searchMemories(
        ...
        try {
            ...
            // 使用原始向量搜索获取候选集
            const candidates = await this.vectorStore.search(userId, queryEmbedding, candidateLimit);
            ...
            const scoredCandidates = candidates.map(memory => {
                ...
                const rawImportance = memory.metadata.importance || 1;
                ...
                const daysPassed = Math.max(0, (now.getTime() - createdAt.getTime()) / oneDayMs);
                ...
                const finalScore = (similarity * 0.7) + (importanceScore * 0.2) + (recencyScore * 0.1);
            ...
            this.logger.debug(`[MemorySearch] Query: "${query}" -> Top ${results.length} results (from ${candidates.length} candidates)`);
            if (results.length > 0) {
                this.logger.debug(`[MemorySearch] Top Match: Score=${results[0]._score.toFixed(3)} (Sim=${results[0]._similarity.toFixed(3)}, Imp=${results[0].metadata.importance}, Age=${results[0]._debug.daysPassed}d)`);
            }
...
`;
            ...
            const workerConfig: DistillationConfig = {
                VERTEX_PROJECT_ID: this.configService.get<string>('VERTEX_PROJECT_ID') || '',
                VERTEX_LOCATION: this.configService.get<string>('VERTEX_LOCATION') || 'us-central1',
            ...
            const output = await this.workerPool.run<DistillationPayload, DistillationResult>(
                'distillConversation',
                {
                    config: workerConfig,
                    prompt: prompt,
