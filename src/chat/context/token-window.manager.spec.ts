...
describe('TokenWindowContextManager', () => {
    ...
    it('should count tokens correctly for mixed content', async () => {
        ...
        const mockMessages = [
            { id: '2', content: '你好', role: ChatRole.USER, createdAt: new Date(2) }, // 2 tokens
            { id: '1', content: 'Hello', role: ChatRole.USER, createdAt: new Date(1) }, // 2 tokens (ceil(1.25) = 2)
        ] as ChatMessage[];
    ...
    it('should handle pure English correctly', async () => {
        // "1234" = 4 * 0.25 = 1 token
        const mockMessages = [
            { id: '3', content: '1234', role: ChatRole.USER, createdAt: new Date(3) },
            { id: '2', content: '5678', role: ChatRole.ASSISTANT, createdAt: new Date(2) },
            { id: '1', content: '9012', role: ChatRole.USER, createdAt: new Date(1) },
        ] as ChatMessage[];
    ...
    it('should handle pure CJK correctly', async () => {
        // "测试" = 2 tokens
        const mockMessages = [
            { id: '2', content: '测试', role: ChatRole.USER, createdAt: new Date(2) }, // 2 tokens
            { id: '1', content: '数据', role: ChatRole.USER, createdAt: new Date(1) }, // 2 tokens
        ] as ChatMessage[];
