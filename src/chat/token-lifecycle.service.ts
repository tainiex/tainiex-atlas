...
export class TokenLifecycleService {
    ...
    scheduleRefreshNotification(client: Socket, token: string) {
        try {
            ...
            if (notifyTime > 0) {
                this.logger.log(`Scheduling token refresh notification for client ${client.id} in ${Math.round(notifyTime / 1000)}s`);
                ...
                this.logger.warn(`Token for client ${client.id} is already in refresh buffer period`);
                this.notifyClient(client, (expiryTime - now) / 1000);
            }
        } catch (error) {
            this.logger.error(`Failed to schedule token refresh for ${client.id}`, error);
        }
    }
    ...
    async handleTokenRefreshed(client: Socket, newToken: string) {
        ...
        try {
            ...
            this.logger.error(`Failed to verify refreshed token for ${client.id}`, error);
