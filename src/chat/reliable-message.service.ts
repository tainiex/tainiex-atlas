...
export class ReliableMessageService {
    ...
    async sendReliableMessage(
        ...
        setTimeout(() => this.checkTimeout(userId, messageId), this.MESSAGE_TIMEOUT);
    }
    ...
    resendPending(client: Socket, userId: string) {
        ...
        this.logger.log(`Resending ${messages.length} pending messages to user ${userId}`);
        ...
        for (const msg of messages) {
            if (now - msg.timestamp < this.MESSAGE_TIMEOUT) {
                ...
                if (msg.retryCount <= msg.maxRetries) {
                    ...
                    this.logger.warn(`Dropping message ${msg.id} for user ${userId} after max retries`);
                }
            }
        }
