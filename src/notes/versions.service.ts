...
export class VersionsService {
     ...
     * 获取特定块的历史记录。
     */
    async getBlockHistory(blockId: string, limit: number = 20): Promise<BlockVersion[]> {
     ...
     * 将块回滚到特定版本。
     */
    async rollbackBlock(blockId: string, versionId: string, userId: string): Promise<Block> {
        ...
        if (version.content !== null) {
            ...
            if (version.diff.content !== undefined) block.content = version.diff.content;
            if (version.diff.metadata !== undefined) block.metadata = version.diff.metadata;
        }
    ...
    async handleDailySnapshots() {
        ...
        for (const note of notesToSnapshot) {
            try {
                ...
                console.error(`[VersionsService] Failed to snapshot note ${note.id}:`, error.message);
            }
        }
    }
