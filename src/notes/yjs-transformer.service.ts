...
export class YjsTransformerService {
    ...
    async syncToBlocks(noteId: string, doc: Y.Doc): Promise<void> {
        ...
        this.logger.debug(`[Debug] Note ${noteId} - Top-level keys: ${keys.join(', ')}`);
        ...
        if (keys.includes('default')) {
            try {
                this.logger.debug(`[Debug] Found 'default' key, attempting to parse as XmlFragment...`);
                ...
                this.logger.debug(`[Debug] Parsed ${blocksToSave.length} blocks from XmlFragment.`);
        ...
        if (blocksToSave.length === 0) {
            this.logger.warn(`[Debug] No blocks extracted from note ${noteId}. Keys: ${keys.join(', ')}`);
            return;
        }
        ...
        const note = await this.noteRepository.findOne({ where: { id: noteId }, select: ['userId', 'lastEditedBy'] });
        const defaultCreator = note?.userId || note?.lastEditedBy;

        const currentBlockIds = new Set(blocksToSave.map(b => b.id).filter(id => !!id));
        ...
        const savePromises = blocksToSave.map(async (partialBlock) => {
            ...
            if (partialBlock.id && existingMap.has(partialBlock.id)) {
                ...
                if (existingBlocks.length > 0) {
                    ...
                    this.logger.warn(`[Debug] No creator found for block in note ${noteId}`);
                }
            }
    ...
    private parseYArrayToBlocks(noteId: string, yBlocks: Y.Array<Y.Map<any>>): Partial<Block>[] {
    ...
    private parseXmlFragmentToBlocks(noteId: string, fragment: Y.XmlFragment): Partial<Block>[] {
        const blocks: Partial<Block>[] = [];
        let index = 0;
        ...
        for (let i = 0; i < fragment.length; i++) {
            ...
            if (child instanceof Y.XmlElement) {
                ...
                if (type) {
                    ...
                    if (!id) {
                        this.logger.debug(`[Debug] Node ${nodeName} at index ${i} has no ID. Will be re-created.`);
                    }
    ...
    private mapNodeNameToBlockType(nodeName: string): BlockType | undefined {
        switch (nodeName) {
            ...
            case 'heading': return BlockType.HEADING1; // Need to check 'level' attr for H1/H2/H3
            case 'image': return BlockType.IMAGE;
            case 'codeBlock': return BlockType.CODE;
            case 'bulletList': return BlockType.BULLET_LIST;
            case 'orderedList': return BlockType.NUMBERED_LIST;
            case 'taskList': return BlockType.TODO_LIST;
            case 'taskItem': return BlockType.TODO_ITEM;
            case 'blockquote': return BlockType.QUOTE;
            case 'horizontalRule': return BlockType.DIVIDER;
            case 'callout': return BlockType.CALLOUT;
            case 'toggle': return BlockType.TOGGLE;
            default: return BlockType.TEXT; // Fallback? or undefined
        }
    }
    ...
    async loadBlocksToYDoc(noteId: string, doc: Y.Doc): Promise<void> {
        ...
        if (!doc.share.has('blocks')) {
            ...
            if (blocks.length > 0) {
                this.logger.log(`[Reconstruct] Reconstructing ${blocks.length} blocks from SQL to Y.Doc 'blocks' for note ${noteId}...`);
                ...
                this.logger.log(`[Reconstruct] Verification: 'blocks' length is now ${fragment.length}`);
                this.logger.log(`[Reconstruct] Verification: 'blocks' length is now ${fragment.length}`);
            } else {
                // Fallback: If SQL is empty, check if 'default' key has legacy data
                if (doc.share.has('default')) {
                    this.logger.log(`[Reconstruct] No SQL blocks found, but 'default' key exists. Migrating 'default' -> 'blocks' in-memory...`);
                    ...
                    if (defaultXml.length > 0) {
                        doc.transact(() => {
                            ...
                            this.logger.warn(`[Reconstruct] Legacy data found in 'default'. Triggering immediate persistence to populate SQL.`);
    ...
    private mapBlockTypeToNodeName(type: BlockType): string {
        switch (type) {
            ...
            case BlockType.HEADING1: return 'heading'; // metadata level: 1
            case BlockType.HEADING2: return 'heading';
            case BlockType.HEADING3: return 'heading';
            case BlockType.IMAGE: return 'image';
            case BlockType.CODE: return 'codeBlock';
            case BlockType.BULLET_LIST: return 'bulletList';
            case BlockType.NUMBERED_LIST: return 'orderedList';
            case BlockType.TODO_LIST: return 'taskList';
            case BlockType.TODO_ITEM: return 'taskItem';
            case BlockType.QUOTE: return 'blockquote';
            case BlockType.DIVIDER: return 'horizontalRule';
