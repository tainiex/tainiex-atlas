...
import { CreateBlockDto, UpdateBlockDto, MoveBlockDto, IBlock } from '@shared/index';
...
export class BlocksService {
    ...
    private async createVersion(
        ...
        const versionNumber = await this.blockVersionRepository.count({
            where: { blockId }
        }) + 1;
        ...
        if (useDiff) {
            ...
            if (previousVersion) {
                ...
                const metadataChanged = JSON.stringify(block.metadata) !== JSON.stringify(previousVersion.metadata);
