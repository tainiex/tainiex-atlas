...
export class LlmService implements OnModuleInit {
    ...
    private async getAdapter(modelName?: string): Promise<ILlmAdapter> {
        ...
        this.logger.log(`[LlmService] Created new Adapter for model '${validatedModel}': ${adapter.constructor.name}`);
        return adapter;
    }
    ...
    async listRemoteModels(): Promise<any[]> {
        ...
        try {
            ...
            this.logger.log(`[LlmService] Initializing SDK (ModelGardenServiceClient) with options: ${JSON.stringify({ ...clientOptions, authClient: '[AuthClient]' })}`);
    ...
    async generateContent(prompt: string, modelName?: string): Promise<string> {
        ...
        this.logger.debug(`[LlmService] Generating content using model '${modelName}' via adapter: ${adapter.constructor.name}`);
        return adapter.generateContent(prompt);
    }

    async chat(history: any[], message: string, modelName?: string): Promise<string> {
        const adapter = await this.getAdapter(modelName);
        this.logger.debug(`[LlmService] Chatting using model '${modelName}' via adapter: ${adapter.constructor.name}`);
    ...
    async getEmbeddings(text: string): Promise<number[]> {
        ...
        const modelName = this.configService.get<string>('VERTEX_EMBEDDING_MODEL') || 'text-embedding-004';
        const adapter = await this.getAdapter(modelName);
