...
export class ZaiAdapter implements ILlmAdapter {
  ...
  async initialize(): Promise<void> {
    ...
    this.logger.info(
      `[ZaiAdapter] Initialized Z.ai client for model: ${this.modelName}`,
    );
  ...
  async generateContent(prompt: string): Promise<string> {
    this.logger.debug(
      `[ZaiAdapter] generateContent request: ${prompt.substring(0, 50)}...`,
    );
    try {
      ...
      this.logger.debug(
        `[ZaiAdapter] generateContent response length: ${content.length}`,
      );
      return content;
    } catch (error) {
      this.logger.error(
  ...
  async *streamChat(
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        ...
        this.logger.error(
          '[ZaiAdapter] API Error Details (Stream):',
          String(error),
        );
      }
      throw error;
