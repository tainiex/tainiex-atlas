...
export class AuthService {
    ...
    constructor(
        ...
        this.googleAndroidClientId = this.configService.get<string>('GOOGLE_ANDROID_CLIENT_ID') || '';
        this.googleIosClientId = this.configService.get<string>('GOOGLE_IOS_CLIENT_ID') || '';
    ...
    async googleLogin(dto: GoogleLoginDto, isMobile: boolean = false): Promise<any> {
        let idToken = dto.idToken;

        try {
            // If code is provided, exchange it for tokens (Web Flow)
            if (dto.code) {
                console.log(`[GoogleLogin] Exchanging code for tokens. Code length: ${dto.code.length}, isMobile: ${isMobile}`);
                ...
                try {
                    ...
                    console.error(`[GoogleLogin] Token exchange failed (isMobile=${isMobile}):`, tokenError.message);

                    // If we weren't in mobile mode but got a mismatch, retry as mobile (fallback)
                    if (!isMobile && tokenError.message && tokenError.message.includes('redirect_uri_mismatch')) {
                        console.log('[GoogleLogin] Redirect URI mismatch detected on legacy flow. Retrying with mobile configuration...');
                        try {
                            const mobileClient = new OAuth2Client(
                                this.googleClientId,
                                this.googleClientSecret
                            ...
                            console.log('[GoogleLogin] Retry: Mobile token exchange successful.');
                        } catch (mobileError) {
                            console.error('[GoogleLogin] Retry failed:', mobileError.message);
                            if (mobileError.response?.data) {
                                console.error('[GoogleLogin] Mobile error details:', JSON.stringify(mobileError.response.data));
                            }
                            // If retry failed with invalid_grant, it's likely because the code was consumed by the first attempt.
                            throw tokenError;
                        }
                    } else {
                        throw new UnauthorizedException(`Google token exchange failed: ${tokenError.message}`);
                    }
                }
            }

            if (!idToken) {
                throw new UnauthorizedException('Google login failed: No ID token provided or returned');
            }
            ...
            throw new UnauthorizedException(`Google login process failed: ${error.message}`);
        }
    }
