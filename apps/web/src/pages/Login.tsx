...
const Login = () => {
    ...
    // Safely get MSAL instance (returns null if MsalProvider not available)
    const getMsalInstanceSafe = () => {
        try {
    ...
    const instance = getMsalInstanceSafe();
    const msalAvailable = instance !== null;

    const handleMicrosoftLogin = () => {
        if (!instance) {
            setErrorMsg('Microsoft login requires secure connection (HTTPS or localhost)');
            return;
        }
                    ...
                    {inviteRequired ? (
                        ...
                        <div className="login-actions">
                            ...
                            <button
                                ...
                                title={!msalAvailable ? 'Requires HTTPS or localhost access' : ''}
                                style={{
                                    fontFamily: 'Segoe UI, Roboto, arial, sans-serif',
                                    opacity: msalAvailable ? 1 : 0.5,
                                    cursor: msalAvailable ? 'pointer' : 'not-allowed',
                                }}
                            >
                                <svg
                                    className="social-icon"
                            ...
                            {!msalAvailable && (
                                ...
                                >
                                    Microsoft login requires HTTPS or localhost access
                                </p>
                            )}
                        </div>
                    )}
