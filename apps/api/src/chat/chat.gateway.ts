        ...
        origin: (requestOrigin, callback) => {
            ...
            const allowedOrigins = rawConfig
                ...
                .map(origin => {
                    ...
                    if (trimmed === '*') return { type: 'wildcard' as const, value: trimmed };
            ...
            if (!requestOrigin || isAllowed) {
                ...
                console.warn(`[CORS] Rejected Origin: ${requestOrigin}. Configured: ${rawConfig}`);
...
export class ChatGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
    ...
    async handleConnection(client: AuthenticatedSocket) {
        ...
        if (!token) {
            this.logger.warn('Connection rejected: No token provided in auth, headers, or cookies');
        ...
        actor.subscribe(snapshot => {
            this.logger.debug(`[${client.id}] State: ${JSON.stringify(snapshot.value)}`);
        ...
        try {
            for await (const event of stream) {
                ...
                if (!currentStreamInfo) {
                    this.logger.warn(`Stream info lost for session ${sessionId}, aborting`);
                    break;
                }
                ...
                const socketsMap = this.server.sockets as Map<string, AuthenticatedSocket>;
            ...
            const updatedSession = await this.chatService.getSession(sessionId, user.sub);

            // Find current client for done event
            const finalStreamInfo = this.activeStreams.get(sessionId);
            const socketsMap = this.server?.sockets as Map<string, AuthenticatedSocket> | undefined;
            const finalClient =
