...
import { ChatRole, CHAT_ROOT_PARENT_ID, ChatStreamEvent } from '@tainiex/shared-atlas';
...
export class ChatService {
    ...
    async *streamMessage(
        ...
        this.logger.debug(`[ChatService] Distillation Progress: ${msgCount}/${DISTILL_THRESHOLD}`);

        if (shouldDistill) {
            this.logger.debug('[ChatService] Triggering Memory Distillation (Buffered 10 msgs)');
            ...
            void (async () => {
                try {
                    ...
                    this.logger.error('[ChatService] Distillation trigger failed', String(e));
                }
            })();
        }
        ...
        if (shouldUpdateTitle) {
            ...
            titlePromise = this.updateSessionTitle(session);
        } else {
            this.logger.debug(
...
`;
        ...
        try {
            ...
            if (error instanceof Error) {
                ...
                this.logger.error('[Agent] Execution failed (unknown error)', String(error));
                yield {
                    type: 'error',
    ...
    private async updateSessionTitle(session: ChatSession) {
        try {
...
Title:`;
    ...
    async regenerateAllTitles() {
        ...
        this.logger.debug(`[ChatService] Found ${sessions.length} sessions with default title.`);
    ...
    async *updateMessageAndRegenerate(
        ...
        if (aiReply) {
            this.logger.debug('[ChatService] Found existing AI reply, archiving and deleting...');
        ...
        try {
            ...
            this.logger.error('[ChatService] Stream AI Error:', errorStack || errorMessage);
