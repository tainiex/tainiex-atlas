import { Controller, Post, UseGuards, Req, HttpException, HttpStatus, Param } from '@nestjs/common';
...
export class UploadController {
     ...
     */
    @Post('image/:noteId')
    async uploadImage(@Req() req: RequestWithUser, @Param('noteId') noteId: string) {
     ...
     */
    @Post('video/:noteId')
    async uploadVideo(@Req() req: RequestWithUser, @Param('noteId') noteId: string) {
        const file = await req.file();
        if (!file) {
            throw new HttpException('No file uploaded', HttpStatus.BAD_REQUEST);
        }
     ...
     */
    @Post('file/:noteId')
    async uploadFile(@Req() req: RequestWithUser, @Param('noteId') noteId: string) {
        const file = await req.file();
        if (!file) {
            throw new HttpException('No file uploaded', HttpStatus.BAD_REQUEST);
        }
