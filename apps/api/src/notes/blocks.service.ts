...
export class BlocksService {
     ...
     * 创建新块。
     */
    async create(noteId: string, userId: string, createBlockDto: CreateBlockDto): Promise<Block> {
        // Check permission
        ...
        if (position === undefined) {
            ...
            const maxPosition = await this.blockRepository
                ...
                .andWhere('block.parentBlockId IS NULL OR block.parentBlockId = :parentBlockId', {
     ...
     * 更新块。
     */
    async update(id: string, userId: string, updateBlockDto: UpdateBlockDto): Promise<Block> {
     ...
     * 移动块到新位置。
     */
    async move(id: string, userId: string, moveBlockDto: MoveBlockDto): Promise<Block> {
        const block = await this.blockRepository.findOne({ where: { id } });
        if (!block) {
            throw new NotFoundException('Block not found');
        }
        ...
        if (useDiff) {
            ...
            if (previousVersion) {
                ...
                const metadataChanged =
                    JSON.stringify(block.metadata) !== JSON.stringify(previousVersion.metadata);
