...
export class YjsService {
    ...
    // 跟踪待持久化的更新（批量写入）
    private pendingUpdates = new Map<string, { doc: Y.Doc; updateCount: number }>();
     ...
     * 获取状态更新（用于初始同步）。
     */
    async getStateAsUpdate(noteId: string, stateVector?: Uint8Array): Promise<Uint8Array> {
        const doc = await this.getDocument(noteId);
    ...
    private async persistPendingUpdates(): Promise<void> {
        ...
        for (const [noteId, { doc }] of updates) {
            try {
                ...
                console.error(`[YjsService] Failed to persist document ${noteId}:`, error);
