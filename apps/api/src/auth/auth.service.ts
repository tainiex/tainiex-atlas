...
export class AuthService {
    ...
    async googleLogin(dto: GoogleLoginDto, isMobile: boolean = false): Promise<AuthResult> {
        ...
        try {
                ...
                try {
                    ...
                    if (
                        ...
                        try {
                            ...
                            console.error('[GoogleLogin] Retry failed:', error.message);
            ...
            throw new UnauthorizedException(`Google login process failed: ${errorMessage}`);
        }
    }

    private async processGoogleLogin(googlePayload: TokenPayload): Promise<AuthResult> {
    ...
    async microsoftLogin(idToken: string): Promise<AuthResult> {
        ...
        // 2. Extract user info
        const { oid, email, preferred_username } = payload;
    ...
    async microsoftSignup(invitationCode: string, signupToken: string): Promise<AuthResult> {
    ...
    async googleSignup(invitationCode: string, signupToken: string): Promise<AuthResult> {
