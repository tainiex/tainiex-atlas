...
export class ActivityGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
    @WebSocketServer()
    server: Server;
    ...
    handleActivityEvent(event: ActivityEvent) {
        this.server.to(`session:${event.sessionId}`).emit('activity:status', event.payload);
        this.logger.verbose(
