...
describe('AgentEvent <-> ChatStreamEvent Type Compatibility', () => {
    ...
    describe('Exhaustive Type Coverage', () => {
        it('should handle all AgentEvent types in mapping function', () => {
            const mapEventToChatStream = (event: AgentEvent): ChatStreamEvent | null => {
                switch (event.type) {
                    case 'thought':
                    case 'tool_call':
                    case 'tool_result':
            ...
            // Test samples
            expect(mapEventToChatStream({ type: 'thought', content: 'test' })).toEqual({
                type: 'thought',
                content: 'test',
            });

            expect(mapEventToChatStream({ type: 'final_answer', content: 'done' })).toBeNull();
        });
    });
    ...
    describe('Field Compatibility', () => {
        ...
        it('should have compatible tool fields', () => {
            ...
            if (agentToolCall.type === 'tool_call' && chatToolCall.type === 'tool_call') {
