...
export class ReactAgentEngine implements IAgentEngine {
    ...
    async *execute(config: AgentRunConfig): AsyncGenerator<AgentEvent> {
        ...
        while (loopCount < maxLoops) {
            ...
            const llmStream = this.llm.streamChat(messagesForLLM, effectiveModel, toolsForLLM, {
                signal,
            });
            ...
            if (hasToolCall) {
                console.log('[ReactAgentEngine] Parsing buffered JSON for tool call...');
                ...
                console.log('[ReactAgentEngine] Skipping tool call parsing (text response)');
            }
            ...
            if (toolCall) {
                ...
                if (toolInstance) {
                    ...
                    try {
                        ...
                        console.log('[ReactAgentEngine] ✗ Tool execution error:', error.message);
