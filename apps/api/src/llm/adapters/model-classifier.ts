...
export class ModelClassifier {
    ...
    static validateModel(modelName?: string): LlmModelName {
        if (modelName && Object.prototype.hasOwnProperty.call(MODEL_CONFIG, modelName)) {
    ...
    static getModelCategory(modelName: string): 'preview' | 'ga' | LlmProvider {
        ...
        if (config) {
            if (config.provider === LlmProvider.OPENROUTER) return LlmProvider.OPENROUTER;
    ...
    private static generateFallbackDescription(modelName: string): string {
        ...
        return category === 'preview' ? 'Preview model (v1beta1 API)' : 'GA model (VertexAI SDK)';
    }
}
