...
describe('AuthController (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        invitationRepo = moduleFixture.get<Repository<InvitationCode>>(getRepositoryToken(InvitationCode));
    });
    ...
    it('should register a new user, login, and access profile', async () => {
        ...
        const codeEntity = await invitationRepo.findOne({ where: { isUsed: false } });
        if (!codeEntity) {
            throw new Error('No available invitation code for testing. ensureInvitationCodes failed?');
        }
